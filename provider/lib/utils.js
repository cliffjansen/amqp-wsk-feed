...
var amqpUtils = require('./amqpUtils.js');
...
module.exports = function(logger, triggerDB, redisClient) {
    ...
    this.createTrigger = function(triggerIdentifier, newTrigger) {
        return this.createAmqpTrigger(triggerIdentifier, newTrigger);
    }

    this.createAmqpTrigger = function(triggerIdentifier, newTrigger) {
        var method = 'NEWcreateTrigger';

        var callback = function fireCallback() {
        ...
        return this.amqpUtils.createFeed(newTrigger, callback);
    }

    this.originalCreateTrigger = function(triggerIdentifier, newTrigger) {
    ...
    this.fireTrigger = function(dataTrigger) {
        ...
        var delivery = dataTrigger.amqpDelivery;


//        logger.info(method, 'Alarm fired for', triggerIdentifier, 'attempting to fire trigger');
        if (delivery) {
            logger.info(method, 'AMQP message event fired for', triggerIdentifier, 'attempting to fire trigger');
        }
        else {
            logger.info(method, 'AMQP link error event fired for', triggerIdentifier, 'attempting to fire trigger');
        }
        ...
        // TODO: disable trigger if link error here?  earlier?
    };
    ...
    this.stopTrigger = function (triggerIdentifier) {
        ...
        if (utils.triggers[triggerIdentifier]) {
            ...
            else if (utils.triggers[triggerIdentifier].address) {
                utils.amqpUtils.deleteReceiver(triggerIdentifier);
            }
    ...
    this.setupFollow = function(seq) {
        ...
        try {
            var feed = triggerDB.follow({
                since: seq,
// TODO: restore these lines and confirm
//                include_docs: true,
...
//                query_params: {worker: utils.worker}
            ...
            feed.on('change', (change) => {
                ...
                if (utils.triggers[triggerIdentifier]) {
                    ...
                    if (doc._deleted) {
                        return;         // TODO: check why this test needed by AMQP and not Alarms
                    }
